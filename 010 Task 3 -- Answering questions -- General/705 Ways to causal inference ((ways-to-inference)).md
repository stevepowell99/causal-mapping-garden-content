...
|Process oriented; Process tracing|Takes very few links extremely seriously, where causal mapping takes many links from many sources.<br><br>Whether its tests can be run over a corpus of many accounts rather than one case is the subject of [a working paper](/theory-fit/) in the [Rubicon](/rubicon/) chapter.|One case, studied in depth.<br><br>A mechanism broken into parts, each of which could be false on its own.<br><br>Predictions fixed **before** the search: what each part should have left behind, and what would be there instead if a named rival held.|This mechanism did or did not run in this case. Says nothing about any other case.<br><br>Evidence is weighed on two properties: how surely it would appear if the hypothesis held, and how unlikely it would be if a rival held. The four tests, straw in the wind, hoop, smoking gun and doubly decisive, are names for combinations of those two.|The detective|Van Evera; Bennett; Beach &amp; Pedersen; Collier.<br><br>In evaluation, Befani &amp; Mayne|
