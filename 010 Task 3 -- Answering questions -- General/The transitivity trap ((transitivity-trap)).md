...
[[882 Granularity, generalisability and chunking are coding problems for causal mapping too ((granularity))]]
[[Source clustering]]   
...
The answer is of course that we cannot because the first part only makes sense for pig farmers, and the second part only makes sense for wheat farmers. In general, from G à C (in context P) and C à S (in context W), we can only conclude that G à S in the intersection of the contexts P and W. Correctly making inferences about indirect effects is the key benefit but also the key challenge for any approach which uses causal diagrams or maps, including quantitative approaches [@bollenTotalDirectIndirect1987].
...
![notion image](img/map-notion-image.gif)
