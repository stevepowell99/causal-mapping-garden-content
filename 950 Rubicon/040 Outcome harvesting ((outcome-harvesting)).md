...
flowchart TD
  harv("Harvest workshop<br/>people who were there<br/>draft the outcomes")
  draft("Drafting loop<br/>two or three rounds each")
  subst("Substantiation<br/>nominated third parties")
  tbl("The outcomes table")
  obj("Objectives and theory of change<br/>written down beforehand")
  cls("Classify each outcome<br/>against objectives and actors")
  gap("Coverage: which objectives<br/>have no outcome against them")
  sense("Sense-making workshop<br/>reads the pattern and decides")
  harv --> draft --> subst --> tbl
  tbl --> cls
  obj --> cls
  cls --> gap
  gap --> sense
  cls --> sense
  classDef people fill:#EAF7F8,stroke:#6DC4C8,stroke-width:2px,color:#1F1F36;
  classDef machine fill:#FFFFFF,stroke:#1F1F36,stroke-width:1px,color:#1F1F36,stroke-dasharray:4 3;
  classDef out fill:#6DC4C8,stroke:#1F1F36,stroke-width:2px,color:#1F1F36;
  class harv,draft,subst,sense people
  class cls,gap machine
  class tbl,obj out
```
