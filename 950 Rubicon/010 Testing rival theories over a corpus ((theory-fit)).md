...
- **Measuring a test's discriminating power rather than asserting it**, by generating synthetic interviews in which each theory is true by construction and reading the resulting matrix. This is the likelihood ratio of Bayesian process tracing, estimated by experiment instead of by expert judgement, and the standing complaint against Bayesian process tracing in evaluation is exactly that those numbers come from elicitation nobody can defend.
